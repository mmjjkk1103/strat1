...
export function renderSajuReport(saju) {
    ...
    return [
        `<p><strong>사주팔자</strong><br>${pillarLine}</p>`,
        `<p><strong>일간 중심 기질: ${saju.dayMaster.stem} · ${element.name}</strong><br>${element.temperament}</p>`,
        `<p><strong>오행 분포</strong><br>${renderElementSpread(saju.elements)}</p>`,
        `<p><strong>음양 분포</strong><br>양 ${saju.yinYang.yang} / 음 ${saju.yinYang.yin}. ${saju.yinYang.yang >= saju.yinYang.yin ? '밖으로 드러내고 추진하는 결이 조금 더 앞섭니다.' : '안쪽에서 살피고 축적하는 결이 조금 더 앞섭니다.'}</p>`,
        ...
        `<p><strong>해석 기준</strong><br>${saju.calendarNote} ${saju.engineNote}</p>`,
    ].join('');
}
